...
function StakeRow() {
  ...
  const getUser = async () => {
    try {
      const response = await axios.get(apiUrl + "/getTeamListLevel10", {
        params: {
          address: address,
        },
      });
      if (response?.status === 200) {
        ...
        setDirectUser(response?.data?.data?.[0]?.downlineUsers);
      } else {
        setDirectUser([]);
      }
    } catch (error) {
      ...
      <div className="col-xl-12">
        <div className="card custom-card overflow-hidden">
          <div className="card-header justify-content-between">
            <div className="card-title">10 Level Team Data</div>
          </div>
